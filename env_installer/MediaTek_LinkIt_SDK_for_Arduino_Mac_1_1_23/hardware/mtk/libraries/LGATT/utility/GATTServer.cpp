...
VMINT GATTService::addDeclaration(LGATTService *service, VM_ATT_HANDLE srvc_handle, ard_decl_type_enum type)
{
    ...
    for (i = 0; TYPE_END != service->_gatts_service[i].type; i++)
    {  
        if (!service->_gatts_service[i].triggered)
        {
...
void GATTServer::btcm_cb(VMUINT evt, void* param, void* user_data)
{
    ...
    switch (evt)
    {
        case VM_SRV_BT_CM_EVENT_ACTIVATE:
        {
            // BT is turned on
            server->_result = true;
            vm_btcm_srv_set_visibility(VM_SRV_BT_CM_VISIBILITY_OFF);
            server->post_signal();
            break;
        }
...
boolean GATTServer::rEnd(LGATTServerClass *server)
{
    ...
    else if (VM_SRV_BT_CM_POWER_ON == vm_btcm_get_power_status())
    {
        ...
        {
            if ((server->_gatts_service[i] != NULL) && (server->_gatts_service[i]->_gatts_srv_ctx != NULL))
            {
                gatts_srv_ctx_t *srv_ctx = (gatts_srv_ctx_t *)server->_gatts_service[i]->_gatts_srv_ctx;
                if (srv_ctx->reg_ctx)
                {
                    vm_gatts_deregister(srv_ctx->reg_ctx);
                }
            }
        }

        if (vm_btcm_srv_get_visibility() == VM_SRV_BT_CM_VISIBILITY_OFF)
        {
            vm_btcm_switch_off();
            return false;
        }
